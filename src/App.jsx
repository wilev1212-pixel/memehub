...
export default function App() {
  ...
  const [adminTab, setAdminTab] = useState('memes');
  ...
  const doLogin = async () => {
    ...
    setIsAdmin(res[0].is_admin === true);
  ...
  const doLogout = () => {
    ...
    setUsername(null); setIsAdmin(false); setVotes({}); setFollows({}); setCVotes({});
    setView("feed");
  };
  ...
  return (
    <div style={S.root}>
      ...
      <div style={S.nav}>
        ...
        {isAdmin && (
          <button style={S.navBtn(view==="admin")} onClick={()=>{ setView("admin"); }}>
      ...
      {view === "admin" && isAdmin && (() => {
        ...
        return (
          <div style={{ padding:"14px", maxWidth:1000, margin:"0 auto" }}>
            <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:26, color:"#ff4444", letterSpacing:3, marginBottom:4 }}>⚙️ PANNEAU ADMIN</div>
            <div style={{ fontSize:11, color:"#444", marginBottom:16 }}>Connecté en tant que @{username} · Tous les droits</div>

            {/* Stats */}
            <div style={{ display:"grid", gridTemplateColumns:"repeat(4,1fr)", gap:8, marginBottom:16 }}>
              {[
                ["MÈMES",memes.length,"📸"],
                ["UTILISATEURS",[...new Set(memes.map(m=>m.author))].length,"👥"],
                ["COMMENTAIRES",allComments.length,"💬"],
                ["LIKES TOTAL",memes.reduce((a,m)=>a+m.likes,0),"❤️"],
              ].map(([l,v,e])=>(
                <div key={l} style={{ background:"#0f0f0f", border:"1px solid #1a1a1a", borderRadius:8, padding:"12px 14px" }}>
                  <div style={{ fontSize:20 }}>{e}</div>
                  <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:28, color:AC }}>{v}</div>
            ...
            <div style={{ display:"flex", gap:6, marginBottom:12 }}>
              {[["memes","📸 MÈMES"],["users","👥 UTILISATEURS"],["comments","💬 COMMENTAIRES"]].map(([k,l])=>(
                <button key={k} style={{ background:adminTab===k?AC:"#111", border:`1px solid ${adminTab===k?AC:"#222"}`, borderRadius:5, color:adminTab===k?"#080808":"#555", fontFamily:"'Bebas Neue',sans-serif", fontSize:14, padding:"5px 14px", cursor:"pointer" }}
            ...
            {adminTab==="memes" && (
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                {[...memes].sort((a,b)=>new Date(b.created_at||b.timestamp)-new Date(a.created_at||a.timestamp)).map(m=>(
                  <div key={m.id} style={{ background:"#0f0f0f", border:"1px solid #1a1a1a", borderRadius:8, padding:"10px 14px", display:"flex", alignItems:"center", gap:12 }}>
                    <div style={{ width:60, height:40, borderRadius:5, overflow:"hidden", flexShrink:0, background:isBg(m.imageUrl)?getBg(m.imageUrl):"#111" }}>
                      {!isBg(m.imageUrl)&&<img src={m.imageUrl} alt="" style={{ width:"100%", height:"100%", objectFit:"cover" }} />}
                    </div>
                    <div style={{ flex:1, minWidth:0 }}>
                      <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:14, color:"#e8e8e8", overflow:"hidden", textOverflow:"ellipsis", whiteSpace:"nowrap" }}>{m.title}</div>
                      <div style={{ fontSize:10, color:"#555" }}>@{m.author} · ▲{m.likes} ▼{m.dislikes} · 💬{(comments[m.id]||[]).length} · {timeAgo(m.created_at||m.timestamp)}</div>
                    </div>
                    <button style={{ background:"#1a0000", border:"1px solid #3a0000", borderRadius:5, color:"#ff4444", fontFamily:"'Bebas Neue',sans-serif", fontSize:12, padding:"4px 10px", cursor:"pointer", flexShrink:0 }}
                      onClick={()=>{ if(window.confirm(`Supprimer "${m.title}" ?`)) deleteMeme(m.id); }}>🗑️ SUPPR</button>
                  </div>
                ))}
              </div>
            )}
            ...
            {adminTab==="users" && (
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                {[...new Set(memes.map(m=>m.author))].map(u=>{
                  const um = memes.filter(m=>m.author===u);
                  return (
                    <div key={u} style={{ background:"#0f0f0f", border:"1px solid #1a1a1a", borderRadius:8, padding:"10px 14px", display:"flex", alignItems:"center", gap:12 }}>
                      <Av name={u} size={36} />
                      <div style={{ flex:1 }}>
                        <div style={{ fontFamily:"'Bebas Neue',sans-serif", fontSize:14, color:"#e8e8e8" }}>@{u}</div>
                        <div style={{ fontSize:10, color:"#555" }}>{um.length} mèmes · {um.reduce((a,m)=>a+m.likes,0)} likes</div>
                      </div>
                      {u !== username && (
                        <button style={{ background:"#1a0000", border:"1px solid #3a0000", borderRadius:5, color:"#ff4444", fontFamily:"'Bebas Neue',sans-serif", fontSize:12, padding:"4px 10px", cursor:"pointer" }}
                          onClick={async ()=>{
                            if(!window.confirm(`Supprimer @${u} et tous ses mèmes ?`)) return;
                            const userMemes = memes.filter(m=>m.author===u);
                            for(const m of userMemes) await deleteMeme(m.id);
                            await db.deleteUser(u);
                            showToast(`@${u} supprimé`);
                          }}>🗑️ SUPPR</button>
                      )}
                    </div>
                  );
            ...
            {adminTab==="comments" && (
              <div style={{ display:"flex", flexDirection:"column", gap:6 }}>
                {allComments.sort((a,b)=>b.timestamp-a.timestamp).map(c=>(
                  <div key={c.id} style={{ background:"#0f0f0f", border:"1px solid #1a1a1a", borderRadius:8, padding:"10px 14px" }}>
                    {editComment===c.id
                      ? <div style={{ display:"flex", gap:8 }}>
                          <input style={{ ...S.inp, flex:1 }} value={editCommentText} onChange={e=>setEditCommentText(e.target.value)} />
                          <button style={{ background:AC, border:"none", borderRadius:5, color:"#080808", fontFamily:"'Bebas Neue',sans-serif", fontSize:13, padding:"4px 12px", cursor:"pointer" }}
                            onClick={async ()=>{
                              setCmts(p=>({ ...p, [c.memeId]: (p[c.memeId]||[]).map(x=>x.id===c.id?{...x,text:editCommentText}:x) }));
                              await db.updateComment(c.id, editCommentText);
                              setEditComment(null); showToast("Commentaire modifié ✓");
                            }}>✓ OK</button>
                          <button style={{ background:"transparent", border:"1px solid #333", borderRadius:5, color:"#555", fontFamily:"inherit", fontSize:12, padding:"4px 10px", cursor:"pointer" }}
                            onClick={()=>setEditComment(null)}>Annuler</button>
                        </div>
                      : <div style={{ display:"flex", alignItems:"flex-start", gap:10 }}>
                          <div style={{ flex:1 }}>
                            <div style={{ fontSize:10, color:"#555", marginBottom:4 }}>
                              <span style={{ color:AC }}>@{c.author}</span> · sur mème <span style={{ color:"#777" }}>{memes.find(m=>m.id===c.memeId)?.title||c.memeId}</span> · {timeAgo(c.timestamp)}
                            </div>
                            <p style={{ fontSize:13, color:"#ccc", margin:0 }}>{c.text}</p>
                          </div>
                          <div style={{ display:"flex", gap:4, flexShrink:0 }}>
                            <button style={{ background:"#0a1a0a", border:"1px solid #1a3a1a", borderRadius:5, color:"#44aa44", fontFamily:"'Bebas Neue',sans-serif", fontSize:12, padding:"4px 8px", cursor:"pointer" }}
                              onClick={()=>{ setEditComment(c.id); setEditCommentText(c.text); }}>✏️ EDIT</button>
                            <button style={{ background:"#1a0000", border:"1px solid #3a0000", borderRadius:5, color:"#ff4444", fontFamily:"'Bebas Neue',sans-serif", fontSize:12, padding:"4px 8px", cursor:"pointer" }}
                              onClick={async ()=>{
                                if(!window.confirm("Supprimer ce commentaire ?")) return;
                                setCmts(p=>({ ...p, [c.memeId]: (p[c.memeId]||[]).filter(x=>x.id!==c.id) }));
